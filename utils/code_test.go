...
func TestCodeGenCmdNewProjectWritesAndCleansUpFiles(t *testing.T) {
	...
	expected := "newProject"
	received := localProfile.OMGD.Name

	if expected != received {
		testLogComparison(expected, received)

		LogDebug("Profile didn't update with game name in static folder")
		t.Fail()
	}

	omgdProfile := GetProfile("static/test/newProject/profiles/omgd.yml")
	expected = "newProject"
	received = omgdProfile.OMGD.Name

	if expected != received {
		testLogComparison(expected, received)

		LogDebug("Top level OMGD Profile didn't update with game name in static folder")
		t.Fail()
	}
	...
	testFileShouldExist(t, "static/test/newProject/game/dist/.gitkeep")
}
...
func TestCodeGenCmdOMGDChannelCreation(t *testing.T) {
	...
	testForFileAndRegexpMatch(
		t,
		"static/test/newProject/servers/central/data/modules/match_channel.lua",
		`match_channel_size`,
	)
	...
	testForFileAndRegexpMatch(
		t,
		"static/test/newProject/servers/central/data/modules/match_channel_manager.lua",
