...
gcloud compute scp --project ${GCP_PROJECT} --zone ${GCP_ZONE} --force-key-file-overwrite ../../../server/docker-compose.yml ${OMGD_PROJECT}-omgd-dev-instance-${OMGD_PROFILE}:
...
gcloud compute scp --project ${GCP_PROJECT} --zone ${GCP_ZONE} --recurse --force-key-file-overwrite ../../../server/nakama ${OMGD_PROJECT}-omgd-dev-instance-${OMGD_PROFILE}:
