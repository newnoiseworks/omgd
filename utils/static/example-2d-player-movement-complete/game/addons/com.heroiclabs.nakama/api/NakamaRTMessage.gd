...
class ChannelJoin:
	...
	enum ChannelType { Room = 1, DirectMessage = 2, Group = 3 }
	...
	func _to_string():
		return (
			"ChannelJoin<persistence=%s, hidden=%s, target=%s, type=%d>"
			% [persistence, hidden, target, type]
		)
...
class ChannelLeave:
	...
	const _SCHEMA = {"channel_id": {"name": "channel_id", "type": TYPE_STRING, "required": true}}
...
class ChannelMessageUpdate:
	...
	func _to_string():
		return (
			"ChannelMessageUpdate<channel_id=%s, message_id=%s, content=%s>"
...
class MatchJoin:
	...
	const _SCHEMA = {
		...
		"metadata":
		{"name": "metadata", "type": TYPE_DICTIONARY, "required": false, "content": TYPE_STRING},
	}
...
class MatchLeave:
	...
	const _SCHEMA = {"match_id": {"name": "match_id", "type": TYPE_STRING, "required": true}}
	var match_id: String
...
class MatchDataSend:
	...
	const _SCHEMA = {
		...
		"presences":
		{"name": "presences", "type": TYPE_ARRAY, "required": false, "content": "UserPresences"},
	...
	func _to_string():
		return (
			"MatchDataSend<match_id=%s, op_code=%s, presences=%s, data=%s>"
			% [match_id, op_code, presences, data]
		)
...
class MatchmakerAdd:
	...
	func _init(
		...
		p_min_count: int = 2,
		p_max_count: int = 8,
		p_string_props: Dictionary = Dictionary(),
		p_numeric_props: Dictionary = Dictionary()
	):
		query = p_query
	...
	func _to_string():
		return (
			"MatchmakerAdd<query=%s, max_count=%d, min_count=%d, numeric_properties=%s, string_properties=%s>"
			% [query, max_count, min_count, numeric_properties, string_properties]
		)
...
class MatchmakerRemove:
	...
	const _SCHEMA = {"ticket": {"name": "ticket", "type": TYPE_STRING, "required": true}}
...
class StatusFollow:
	...
	const _SCHEMA = {
		"user_ids":
		{"name": "user_ids", "type": TYPE_DICTIONARY, "required": false, "content": TYPE_STRING},
		"usernames":
		{"name": "usernames", "type": TYPE_DICTIONARY, "required": false, "content": TYPE_STRING},
	}
